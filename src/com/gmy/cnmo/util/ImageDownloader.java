...
public class ImageDownloader {
	 ...
	 */
	public void imageDownload(String url, ImageView mImageView, String path, Context context,
		...
		String imageName = "";
		if (url != null) {
			imageName = SDCardUtil.getInstance().getImageName(url);
		}
		...
		// 文件中也没有，此时根据mImageView的tag，即url去判断该url对应的task是否已经在执行，如果在执行，本次操作不创建新的线程，否则创建新的线程。
			...
			if (mImageView != null) {
				Log.i(TAG, "执行MyAsyncTask --> " + SDCardUtil.flag);
				SDCardUtil.flag++;
				task.execute();
				// 将对应的url对应的任务存起来
				map.put(url, task);
	...
	private Bitmap getBitmapFromFile(Context context, String imageName, String path) {
		...
		if (imageName != null) {
			...
			String real_path = "";
			try {
				if (SDCardUtil.getInstance().hasSDCard()) {
					real_path = SDCardUtil.getInstance().getExtPath()
							+ (path != null && path.startsWith("/") ? path : "/" + path);
				} else {
					real_path = SDCardUtil.getInstance().getPackagePath(context)
							+ (path != null && path.startsWith("/") ? path : "/" + path);
				}
	...
	private class MyAsyncTask extends AsyncTask<String, Void, Bitmap> {
		...
		public MyAsyncTask(String url, ImageView mImageView, String path, Context context,
				OnImageDownload download) {
		...
		protected Bitmap doInBackground(String... params) {
			...
			if (url != null) {
				try {
					...
					String imageName = SDCardUtil.getInstance().getImageName(url);
					if (!setBitmapToFile(path, context, imageName, data)) {
						removeBitmapFromFile(path, context, imageName);
					}
					imageCaches
							.put(url,
									new SoftReference<Bitmap>(data.createScaledBitmap(data, 100,
											100, true)));
				} catch (Exception e) {
					e.printStackTrace();
				}
